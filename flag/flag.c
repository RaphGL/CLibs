...
void flag_free(flag_Parser *p) {
   for (size_t i = 0; i < vec_len_flag(p->flags); i++) {
      struct flag_flag *flag;
      vec_get_flag(p->flags, i, &flag);
      free(flag);
   }

   vec_free_flag(p->flags);
   vec_free_str(p->remaining);
}

size_t flag_nargs(flag_Parser *p) { return vec_len_str(p->remaining); }

char *flag_arg(flag_Parser *p, size_t idx) {
   char *arg;
   if (!vec_get_str(p->remaining, idx, &arg)) {
      return NULL;
   }
...
void flag_print_flags(flag_Parser *p) {
   for (size_t i = 0; i < vec_len_flag(p->flags); i++) {
      ...
      vec_get_flag(p->flags, i, &curr_flag);
...
size_t flag_nflags(flag_Parser *p) { return vec_len_flag(p->flags); }
...
void flag_parse(flag_Parser *p) {
   for (int curr_arg_idx = 0; curr_arg_idx < p->argc; curr_arg_idx++) {
      ...
      if (!arg_is_flag(&arg)) {
         vec_push_str(p->remaining, arg.cstr);
         continue;
      }

      bstr flag_name, flag_value;
      size_t sep_idx = 0;
      if (bstr_index(&sep_idx, arg, bstr_new("=")) || bstr_index(&sep_idx, arg, bstr_new(":"))) {
      ...
      struct flag_flag *curr_flag;
      for (size_t j = 0; j < vec_len_flag(p->flags); j++) {
         vec_get_flag(p->flags, j, &curr_flag);
